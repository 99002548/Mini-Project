...
void insightsofipl::removematch(int mid)        
{
    ...
    for(iter=iplmatches.begin();iter!=iplmatches.end();++iter)
    {
        ...
        {
           insightsofipl::iplmatches.erase(iter);
           break;
        }
    }
}
...
ipl* insightsofipl::findMatchByDate(std::string date, int syear)
{
   ...
   int matches_in_venue(std::string venue)  
   {
    int count=0;
    std::list<ipl> :: iterator iter;
 ...
 int num_of_matches(std::string t1)  //method overloading
   {
    int count=0;
    std::list<ipl> :: iterator iter;
...
int num_of_matches(std::string t1,int syear)  
   {
    int count=0;
    std::list<ipl> :: iterator iter;
...
int num_of_matches_won(std::string t1)  
   {
    int count=0;
    std::list<ipl> :: iterator iter;
...
int num_of_toss_win(std::string t1)
{
